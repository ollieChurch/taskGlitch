    ...
    export default {
        ...
        methods: {
            onSubmit(event) {
                ...
                this.start = new Date(`${sessionFromDate.toDateString()} ${this.fromTime}`)
                this.finish = new Date(`${sessionToDate.toDateString()} ${this.toTime ?? this.fromTime}`)
            ...
            getSchedule(tasks, sessionInMins) {
                ...
                if (!this.toTime) {
                    this.finish.setMinutes(this.start.getMinutes() + totalTaskTime)
                }
