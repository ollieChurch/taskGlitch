<template>
	<div
		...
		:class="{ 'task-card--blocked': task.blocked }"
		role="button"
	...
	>
		<div
			...
			:style="{ backgroundColor: task.blocked ? 'var(--color-warning)' : categoryColor }"
		></div>
		<div class="flex items-center">
			<div class="flex-1 min-w-0 pr-4" :class="{ 'opacity-50': task.blocked }">
				<h5
				...
				>
					<span v-if="!task.completedDateTime" class="inline-flex items-center mr-1 align-middle">
						<Ban v-if="task.blocked" :size="14" class="text-app-warning" />
